...
static char * extract_cookie(request_rec *r, const char *szCookie_name) 
{
    ...
    if (!szRaw_cookie) {
        ap_log_rerror(APLOG_MARK,APLOG_DEBUG|APLOG_NOERRNO, 0,r,ERRTAG  "no cookie found in %s", szRaw_cookie);
	return 0;
    }

    ap_log_rerror(APLOG_MARK,APLOG_DEBUG|APLOG_NOERRNO, 0,r,ERRTAG  "searching for %s in %s", szCookie_name, szRaw_cookie);
    ...
    ap_log_rerror(APLOG_MARK,APLOG_DEBUG|APLOG_NOERRNO, 0,r,ERRTAG  "Found %s in cookie", szCookie);
...
static int Auth_memCookie_check_cookie(request_rec *r)
{
	...
	ap_log_rerror(APLOG_MARK, APLOG_INFO|APLOG_NOERRNO, 0, r, ERRTAG "%s cookie not found %s! not authorized! RemoteIP:%s",szCookieValue, conf->szAuth_memCookie_CookieName, szRemoteIP);
    }
